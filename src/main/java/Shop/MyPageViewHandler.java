...
public class MyPageViewHandler {
    ...
    public void whenOrdered_then_CREATE_1 (@Payload Ordered ordered) {
        ...
        System.out.println("##### listener MyPage Order : " + ordered.toJson());
        System.out.println("##### listener MyPage Order : " + ordered.isMe());
        ...
        try {
            if (shipped.isMe()) {
                ...
                for(MyPage myPage : myPageList){
                    ...
                    myPage.setShipStatus(shipped.getStatus());
                    // view 레파지 토리에 save
                    myPageRepository.save(myPage);
                }
            }
    ...
    public void whenDeliveryCanceled_then_UPDATE_2(@Payload DeliveryCanceled deliveryCanceled) {
        try {
            if (deliveryCanceled.isMe()) {
                ...
                for(MyPage myPage : myPageList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    myPage.setShipStatus(deliveryCanceled.getStatus());
                    // view 레파지 토리에 save
                    myPageRepository.save(myPage);
                }
            }
